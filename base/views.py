...
from .models import Category, Customer, Product, ShoppingCart
...
def profile(request):
    user = request.user
    # Query for all shopping cart items associated with the current user
    shopping_cart_items = ShoppingCart.objects.filter(user=user)

    context = {
        'shopping_cart_items': shopping_cart_items,
        'user_name': user.username,  # Update to use user.username directly
        'user': user,
    }
